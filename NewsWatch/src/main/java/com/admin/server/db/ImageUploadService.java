...
public class ImageUploadService extends HttpServlet implements Servlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		//info.setNewsImageStream(req.getInputStream(), filelength);
		// Retrieve news item details from the request object and set to info object
		UserHelperAdmin helper = new UserHelperAdmin(req,resp,connectionUrl,driverClassName,username,password, tomcatpath);
		helper.setInputstream(inputstream);
		helper.setAllNewsItemFields(newsinfo);
	...
	public void processFormField(FileItem item)
	{
		...
		if (item.isFormField()) {
		    ...
		    if(name.equals("industryTextBox")){
		    	...
		    	//newsinfo.setIndustryName(item.getString());
		    }
		    ...
		    if(name.equals("newstitleTextBox")){
		    	...
		    	//newsinfo.setNewsTitle(item.getString());
		    }
